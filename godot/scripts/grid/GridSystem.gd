...
## Returns all positions within move_range tiles using Dijkstra BFS.
## tiles_dict: { Vector2i: Dictionary } with keys: move_cost, height, blocks_movement
...
static func find_path(
	...
	while open_set.size() > 0:
		...
		for dir in DIRECTIONS_CARDINAL:
			...
			var tentative_g: int = g_score.get(current, 9999999) + step_cost + height_diff
...
## Returns tiles within attack range from origin
