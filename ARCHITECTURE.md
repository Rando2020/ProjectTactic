...
| `godot/scripts/battle/` | Active | Battle flow, tactical combat orchestration |
...
- Save/load to `user://meta-progression.json`.
- Stage reward calculation through `award_stage_rewards(map_id)`.
...
- Start run.
- Generate seeded floor plan through `FloorGenerator`.
- Track current node.
...
- Query `get_current_node()` for display.
...
Seeded roguelite run plan generator.

Handled:

- Run plan creation.
- Floor/node structure.
...
- Put procedural map/floor rules into scenes.

### `BoonDB.gd`

Run boon definitions and weighted option generation.
...
- Three-option boon generation.

Scene responsibility:

- Display options.
- On selection, call `RunManager.add_boon(boon)`.
...
Elite affix and tier generator.

Handled:

- Elite tiers: normal, marked, elite, champion.
- Prefixes such as volatile, fortified, empowered, vampiric.
- Suffixes such as frost, storm, flames, pack, iron, shadows, void, tide.
...
Wanderer encounter definitions.
...
- Rewards and conditions as data.

Scene responsibility:

- Show dialogue.
- Present accept/decline/pay/challenge choices.
- Report outcome to `RunManager` or future encounter resolver.
...
Secret skills taught by wanderers.

Handled:

- Secret skill definitions.
- Unit-specific and neutral secret skill lists.

Scene responsibility:

- Show reward previews.
- Call the future job/ability learning service when a secret skill is earned.

### `JobProgressionDB.gd`

Job level and unlock helper data.
...
- Read `RunManager.get_current_node()`.
...
- Read the current boon node from `RunManager`.
- Display three options from node data or `RunManager.generate_boon_options()`.
- On selection, call `RunManager.add_boon()`.
- Call `RunManager.complete_current_node()`.
- Route to the next node screen.
...
- Display the current wanderer from `RunManager.get_current_node()`.
...
- Award roguelite rewards through `MetaProgression.award_stage_rewards(map_id)` or `RunManager.award_current_node_reward()`.
- Store a reward breakdown so `ResultsScreen.gd` can display both JP/gold and meta-currency.
- Call `RunManager.complete_current_node()` after results flow is complete, not before the player sees rewards.
...
- Use `EliteAffixDB.generate_affix_set()` for battle nodes with elite bonus.
...
1. Keep PR #26 hub scaffold stable.
